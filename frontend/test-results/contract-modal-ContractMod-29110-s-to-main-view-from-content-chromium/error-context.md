...
- Name: contract-modal.spec.ts >> ContractModal — shared >> Back button returns to main view from content
- Location: e2e/contract-modal.spec.ts:53:3
...
Call log:
  - waiting for locator('tr').filter({ has: getByText('nda_alpha_beta.txt') }).getByRole('button', { name: 'View' })
